...
export function TestPage() {
  ...
  const throwUnhandledError = () => {
    throw new Error(
      '🛑 UNHANDLED: This throws an error that crashes the component tree!'
    );
  };
    ...
    <>
      ...
      <CardBlock title="Test Error Handling">
        ...
        <CardContent title="2. Runtime Error (Handled)">
          ...
          <button onClick={throwHandledError}>Trigger Handled Error</button>
        </CardContent>
        <CardContent title="3. Runtime Error (UNHANDLED)">
          ...
          >
            *WARNING: This will crash the component tree and show the Vite error
            overlay.*
          </p>
          <button onClick={throwUnhandledError}>Trigger Unhandled Error</button>
        </CardContent>
        {/* Universal Reset button */}
        <div style={{ marginTop: '20px' }}>
          <button onClick={resetComponent}>Reset All Errors</button>
        </div>
      </CardBlock>
    </>
  );
